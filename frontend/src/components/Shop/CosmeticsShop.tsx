...
export const CosmeticsShop: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'avatar' | 'badge' | 'effect'>('all');
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4">
      ...
      <div className="flex gap-4 mb-8">
        {(['all', 'avatar', 'badge', 'effect'] as const).map((type) => (
          <button
            key={type}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCosmetics.map((cosmetic) => (
          <div key={cosmetic.id} className="bg-white rounded-lg shadow-md hover:shadow-lg p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              {cosmetic.owned ? (
                <span className="bg-green-100 text-green-800 px-4 py-2 rounded-lg font-bold">✓ Owned</span>
              ) : (
                <button
                  onClick={() => handlePurchase(cosmetic.id, cosmetic.price)}
