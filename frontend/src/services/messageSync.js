...
class MessageSyncService {
    ...
    async addPendingMessage(roomId, content, encrypted = false) {
        const message = {
            id: uuidv4(),
            room_id: roomId,
            content,
    ...
    async syncPendingMessages() {
        ...
        this.messageQueue = this.messageQueue.filter(msg => msg.status === 'pending' || msg.status === 'failed');
    }
    async onlineStatusChanged(isOnline) {
    ...
    async getLocalMessages(roomId) {
        const keys = await this.db.keys();
        const messages = [];
        for (const key of keys) {
            if (key.startsWith('msg_')) {
                const msg = await this.db.getItem(key);
